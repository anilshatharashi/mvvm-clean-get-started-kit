...
class GetTasksListUseCaseTest {
    ...
    @Test
    fun execute_success() {
        every { gateway.getOpportunitiesTasks() } returns Single.just(listOf(1,38, 281, 28, 7, 30))

        val testObserver =  useCase.execute(any()).test()
        testObserver.assertComplete()
        testObserver.assertNoErrors()
        testObserver.assertValue(listOf(1,38, 281, 28, 7, 30))
        verify(exactly = 1) { gateway.getOpportunitiesTasks() }
    }

    @Test
    fun execute_error() {
        val exception = Exception()
        every { gateway.getOpportunitiesTasks() } returns Single.error(exception)
